...
def sample_fast_rcnn_targets(boxes, gt_boxes, gt_labels):
    ...
    def sample_fg_bg(iou):
        ...
        #print_num_fg = tf.Print(num_fg, [num_fg])
...
def maskrcnn_loss(mask_logits, fg_labels, fg_target_masks):
    ...
    loss = tf.reduce_mean(loss, name='initial_maskrcnn_loss')
...
@layer_register(log_shape=True)
def secondclassification_head(feature, num_classes):
    """
    Args:
        feature (NxCx7x7):
        num_classes(int): num_category + 1
...
@under_name_scope()
def secondclassification_losses(labels, label_logits):
    """
    Args:
        labels: n,
        label_logits: nxC
    ...
    if config.USE_COCO_AND_MAPILLARY:
        # replace void label by 1 to avoid out of range. Later mask out the contribution of the void label to the loss
        labels_without_void = tf.where(tf.equal(labels, config.VOID_LABEL), tf.ones_like(labels), labels)
    else:
        labels_without_void = labels
    label_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
        labels=labels_without_void, logits=label_logits)
    if config.USE_COCO_AND_MAPILLARY:
        no_void_label_float_mask = tf.cast(tf.not_equal(labels, config.VOID_LABEL), tf.float32)
        label_loss_sum = tf.reduce_sum(label_loss * no_void_label_float_mask)
        n = tf.maximum(tf.reduce_sum(no_void_label_float_mask), 1)
        label_loss = tf.divide(label_loss_sum, n, name='label_loss')
    else:
        label_loss = tf.reduce_mean(label_loss, name='label_loss')

    # to make the handling of void label easier, we don't report these stats when a void label is used for now
    if not config.USE_COCO_AND_MAPILLARY:
        fg_inds = tf.where(labels > 0)[:, 0]
        num_fg = tf.size(fg_inds)
        with tf.name_scope('label_metrics'), tf.device('/cpu:0'):
            prediction = tf.argmax(label_logits, axis=1, name='label_prediction')
            ...
            fg_accuracy = tf.reduce_mean(
                tf.gather(correct, fg_inds), name='fg_accuracy')
        #add_moving_summary(label_loss, accuracy, fg_accuracy, false_negative)
